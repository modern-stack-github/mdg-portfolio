...
function Contact() {
  ...
  const handleSubmit = (event) => {
    ...
    if (form.checkValidity() === true) {
      ...
      emailjs.send(process.env.REACT_APP_EMAILJS_SERVICEID, process.env.REACT_APP_EMAILJS_TEMPLATEID, emailObject, process.env.REACT_APP_EMAILJS_PUBLIC_KEY)
      .then((response) => {
  ...
  return (
    <Container>
      ...
      { showToastContainer && <ToastContainer position="top-end" className="p-3">
        <Toast bg={toastVariant} delay={5000} onClose={() =>  setShowToastContainer(false)} autohide="true">
          <Toast.Header>
